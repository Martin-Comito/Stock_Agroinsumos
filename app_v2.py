...
# CSS ALTO CONTRASTE (NEGRO Y AMARILLO)
st.markdown("""
    <style>
...
def calculadora_stock(key_prefix):
    """Muestra la calculadora estándar y devuelve el total calculado"""
        ...
        with st.container(border=True):
            ...
            if st.button("INGRESAR AL SISTEMA", type="primary"):
                ...
                if user:
                    ...
                    st.toast(f"Hola {user['nombre_completo']} | Sucursal: {st.session_state.usuario_sucursal}", icon="👋")
                    st.rerun()
                else:
...
#  APLICACIÓN PRINCIPAL
    ...
    with st.sidebar:
        ...
        if st.button("Cerrar Sesión", type="secondary"):
            st.session_state.usuario_id = None; st.rerun()
    ...
    # INGRESOS
        ...
        with st.container(border=True):
            ...
            if es_nuevo:
                ...
                if p_map: p_sel = c1.selectbox("Producto (Escriba para buscar)", list(p_map.keys())); prod_id = p_map[p_sel]['id']
                else: es_nuevo = True
    ...
    # ÓRDENES
        ...
        if st.session_state.carrito:
            ...
            # carrito con opción de eliminar
    ...
    # VALIDACIÓN
        ...
        # Solo ver pendientes de MI sucursal
        ...
        if not pend.data: st.info("✅ Nada pendiente.")
            ...
            for item in items:
                with st.container(border=True):
                    ...
                    # CALCULADORA EN VALIDACIÓN
                    # Aquí el usuario suele poner el número final, pero tiene la opcion de calcular
